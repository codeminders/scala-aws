...
object Request {
  ...
  def apply(bucketName: String = "", keyName: String = "", parameters: Seq[(String, String)] = Seq.empty): com.codeminders.scalaws.http.Request = {

    if (bucketName.isEmpty()) {
      new com.codeminders.scalaws.http.Request(new URL("http://s3.amazonaws.com/"))
    } else {
