...
class FunctionalTests extends BasicUnitTest {
  ...
  test("Verify that user can set object's ACL using cannedACL header") {
    val bucket = client.create(randomBucketName)
    removeBucketOnExit(bucket)
  ...
  test("Verify that user can set object's ACL using explicitACL headers") {
    val bucket = client.create(randomBucketName)
    removeBucketOnExit(bucket)
  ...
  test("Verify PUT Object ACL functionality") {
    ...
    val otherReadACLGrant = new Grant(GroupGrantee.AllUsers, Permission.READ_ACP)
    val otherWriteACLGrant = new Grant(GroupGrantee.AllUsers, Permission.WRITE_ACP)
    ...
    //set ACL
    bucket("1").acl = new ACL(defaultACL.owner, List(otherReadACLGrant, otherWriteACLGrant, ownerHasFullControl))
    //check whether ACL has been updated
    val acl = bucket("1").acl
    assert(!acl.owner.uid.isEmpty())
    assert(!acl.owner.displayName.isEmpty())
    assert(acl.grants.size === 3)
    assert(acl.grants.find(g => g == otherReadACLGrant) != None)
    assert(acl.grants.find(g => g == otherWriteACLGrant) != None)
